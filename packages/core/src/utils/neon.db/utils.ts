...
class NeonDBUtils {
  ...
  constructor({
    neonApiKey,
    config: { roleName, dbName, projectName },
    stage,
  }: {
    neonApiKey: string;
    stage: string;
    config: { roleName: string; dbName: string; projectName: string };
  }) {
  ...
  public async getProject() {
    ...
    const project = projects.find(
      (project) => project.name === this.PROJECT_NAME,
    );
  ...
  public async waitForBranchReady(
    projectId: string,
    branchName: string,
  ): Promise<Branch> {
  ...
  public async getOrCreateBranch(
    ...
    try {
      ...
      if (!targetBranch) {
        ...
        targetBranch = await this.waitForBranchReady(
          projectId,
          targetBranchName,
        );
      } else {
