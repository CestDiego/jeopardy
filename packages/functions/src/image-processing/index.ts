...
export const handler = async (event: any) => {
  ...
  if (S3_TRANSFORMED_IMAGE_BUCKET) {
    logger.info(`Uploading transformed image to S3 ${S3_TRANSFORMED_IMAGE_BUCKET}/${originalImagePath}/${operationsPrefix}`);
    await retry(async () => await uploadTransformedImage(transformedImageBuffer, originalImagePath, operationsPrefix, contentType));
  }
...
async function processImage(originalImageBody: Uint8Array, operations: ImageOperations, contentType: string) {
  ...
  if (operations.format) {
    ...
    transformedImage = transformedImage.toFormat(operations.format as keyof FormatEnum, formatOptions);
  }

  return transformedImage;
}

async function uploadTransformedImage(imageBuffer: Buffer, originalImagePath: string, operationsPrefix: string, contentType: string) {
  try {
    ...
    if (upload.$metadata.httpStatusCode === 200) {
      logger.info(`Uploaded transformed image to S3 ${S3_TRANSFORMED_IMAGE_BUCKET}/${originalImagePath}/${operationsPrefix}`);
    } else {
      logger.warn(upload.$metadata,`Upload to S3 might have failed for ${S3_TRANSFORMED_IMAGE_BUCKET}/${originalImagePath}/${operationsPrefix}`);
      throw new Error(`Upload to S3 might have failed for ${S3_TRANSFORMED_IMAGE_BUCKET}/${originalImagePath}/${operationsPrefix}`);
    }
  } catch (error) {
...
async function applyWatermark(image: Sharp.Sharp, operations: ImageOperations) {
  ...
  const watermark = Sharp(Buffer.from(Watermark, 'base64'))
